...
src_prepare() {
	...
	for t_linux in "${WORKDIR}/fpcbuild-3.2.2/fpcsrc/compiler/systems/t_linux.pas"  "${WORKDIR}/fpc-3.2.2/compiler/systems/t_linux.pas"; do 
		sed -i 's/elf64-x86-64/elf/g' "$t_linux" || die
	done
...
src_compile() {
	...
	sed -i 's/-b elf64-x86-64/         -b elf/g' "${WORKDIR}/lib/fpc/3.2.2/ppcx64" || die
