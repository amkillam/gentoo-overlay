...
_git-r3-tor_set_gitdir() {
	...
	if [[ ! -d ${GIT_DIR} ]]; then
		...
		mkdir "${GIT_DIR}" || die
		ALL_PROXY=socks5h://127.0.0.1 git init --bare -b __init__ || die
		if [[ ${saved_umask} ]]; then
...
_git-r3-tor_set_submodules() {
	...
	while read l; do
		...
		local upd=$(echo "${data}" | _git_tor config -f /dev/fd/0 \
		...
		submodules+=(
			"${enc_subname}"
			"$(echo "${data}" | _git_tor config -f /dev/fd/0 \
				submodule."${subname}".url || die)"
			"$(echo "${data}" | _git_tor config -f /dev/fd/0 \
				submodule."${subname}".path || die)"
		)
	done < <(echo "${data}" | _git_tor config -f /dev/fd/0 -l || die)
}
...
git-r3-tor_fetch() {
	...
	for r in "${repos[@]}"; do
		if [[ ! ${EVCS_OFFLINE} ]]; then
			...
			local fetch_command=( _git_tor fetch "${r}" )
			...
			if [[ ${clone_type} == mirror ]]; then
				...
				if [[ ${remote_ref} == HEAD ]]; then
					...
					local tagref=$(_git_tor ls-remote "${r}" "refs/tags/${remote_ref}")
			...
			if [[ ${clone_type} == shallow ]]; then
				if _git-r3-tor_is_local_repo; then
					...
					# bug #491260
					clone_type=single
				elif [[ ! $(_git_tor rev-parse --quiet --verify "${fetch_r}") ]]
				then
			...
			if [[ ${clone_type} == mirror || ${fetch_l} == HEAD ]]; then
				...
				_git_tor symbolic-ref HEAD refs/git-r3/HEAD \
		...
		if [[ ${commit_date} ]]; then
			local dated_commit_id=$(
				_git_tor rev-list --first-parent --before="${commit_date}" \
					-n 1 "${remote_ref}"
			)
			if [[ ${?} -ne 0 ]]; then
				...
				set -- _git_tor update-ref --no-deref "${local_ref}" "${dated_commit_id}"
			fi
		else
			local full_remote_ref=$(
				_git_tor rev-parse --verify --symbolic-full-name "${remote_ref}"
			)

			if [[ ${full_remote_ref} ]]; then
				# when we are given a ref, create a symbolic ref
				# so that we preserve the actual argument
				set -- _git_tor symbolic-ref "${local_ref}" "${full_remote_ref}"
			else
				# otherwise, we were likely given a commit id
				set -- _git_tor update-ref --no-deref "${local_ref}" "${remote_ref}"
			fi
		fi
		...
		if [[ ${EGIT_LFS} ]]; then
			...
			local lfs_fetch_command=( _git_tor lfs fetch "${r}" "${remote_ref}" )
	...
	if _git_tor cat-file -e "${local_ref}":.gitmodules &>/dev/null; then
		local submodules
		_git-r3-tor_set_submodules "${_GIT_SUBMODULE_PATH}" \
			"$(_git_tor cat-file -p "${local_ref}":.gitmodules || die)"
		...
		while [[ ${submodules[@]} ]]; do
			...
			if [[ $(_git_tor ls-tree -d "${local_ref}" "${path}") ]]
			then
				local commit=$(_git_tor rev-parse "${local_ref}:${path}" || die)
...
git-r3-tor_checkout() {
	...
	if ! _git_tor cat-file -e refs/git-r3/"${local_id}"/__main__; then
		die "Logic error: no local clone of ${repos[0]}. git-r3-tor_fetch not used?"
	fi
	local remote_ref=$(
		_git_tor symbolic-ref --quiet refs/git-r3/"${local_id}"/__main__
	)
	local new_commit_id=$(
		_git_tor rev-parse --verify refs/git-r3/"${local_id}"/__main__
	)
	...
	git-r3-tor_sub_checkout() {
		...
		_git_tor init --quiet -b __init__ || die
		if [[ ${EGIT_LFS} ]]; then
			...
			_git_tor lfs install --local --skip-repo || die
		fi
		...
		_git_tor config --global --add safe.directory \
		...
		echo "${@}" >&2
		"${@}" || die "_git_tor checkout ${remote_ref:-${new_commit_id}} failed"
		...
		local has_git_lfs_filters=$(
			_git_tor grep "filter=lfs" -- ".gitattributes" "**/.gitattributes"
		)
	...
	local old_commit_id=$(
		_git_tor rev-parse --quiet --verify refs/git-r3/"${local_id}"/__old__
	)
	if [[ ! ${old_commit_id} ]]; then
		...
		if [[ "${old_commit_id}" != "${new_commit_id}" ]]; then
			...
			set -- _git_tor --no-pager diff --stat \
	...
	_git_tor update-ref --no-deref refs/git-r3/"${local_id}"/{__old__,__main__} || die
...
git-r3-tor_peek_remote_ref() {
	...
	for r in "${repos[@]}"; do
		...
		# split on whitespace
		local ref=(
			$(_git_tor ls-remote "${r}" "${lookup_ref}")
		)
